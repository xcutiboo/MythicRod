package io.xcutiboo.mythicrod.gui.utils;
...
public class ItemBuilder {
    ...
    public ItemBuilder(Material material, int amount) {
        ...
        this.item = new ItemStack(material, Math.clamp(amount, 1, 64));
    ...
    public ItemBuilder addLore(String... lines) {
        if (meta != null && lines != null && lines.length > 0) {
            ...
            for (String line : lines) {
                existingLore.add(LegacyComponentSerializer.legacyAmpersand().deserialize(line));
            }
    ...
    public static ItemStack createGlowing(Material material, String name) {
        return new ItemBuilder(material).name(name).glow(true).build();
    }
}
