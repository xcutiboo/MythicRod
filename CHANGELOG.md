...
- **Local assistant-state ignores** — `.gitignore` now covers `.claude/`, `.cursor/`, `.continue/`, `.cline/`, `.windsurf/`, `.codeium/`, `.aider*`, `.tabnine/`, `.copilot/`, `.history/`, and `*.code-workspace` so contributors using these tools never accidentally commit local session state.
