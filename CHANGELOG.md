...
- **`/mythicrod give` is now Folia-safe** — inventory mutation for the target player previously ran on the command sender's thread. `BrigadierCommandManager` now builds the rod, then dispatches the actual `Inventory#addItem` through `PlatformScheduler.runForPlayer` on the target's owning scheduler, re-checks `target.isOnline()` after the handoff, and delivers success / inventory-full / failure feedback from the target's owner thread.
- **`ExternalDropProvider` slow-call warnings** — `PaperMythicRodAPI.getProviderWeight` and `createExternalItem` now time each call and emit a `Level.WARNING` with the provider key and stage (`getWeight` / `generateItem`) when the call exceeds 50 ms on the fishing hot path. Providers that violate the non-blocking contract are now visibly named in server logs instead of silently stalling fishing.
